    ...
    <div className="flex flex-col h-[calc(100vh-3.5rem)] bg-background rounded-lg border border-border">
      {messages.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center flex-1 px-4">
          <h1 className="text-4xl font-bold mb-12 text-center">What property would you like to analyze?</h1>
          ...
          <ScrollArea className="flex-1 px-4" ref={scrollAreaRef}>
            <div className="max-w-3xl mx-auto py-6 space-y-6">
              {messages.map((message, index) => (
                <ChatMessage key={index} content={message.content} role={message.role} timestamp={message.timestamp} />
              ))}
              {isLoading && (
            ...
            {showActions && (
              <div className="flex justify-center gap-4 py-6 mb-4">
                {onDownloadReport && analysisData && (
                  <Button onClick={handleShowReport} variant="outline" className="gap-2 border-border hover:bg-accent">
                ...
                {onViewAnalytics && (
                  <Button onClick={onViewAnalytics} variant="outline" className="gap-2 border-border hover:bg-accent">
